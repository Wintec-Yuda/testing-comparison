      ...
      <div class="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden">
        <div class="px-6 py-5 border-b border-slate-800 flex items-center justify-between bg-slate-950">
          <div>
            ...
            <p class="text-slate-500 text-sm">Website vs Mobile vs Command Center</p>
          </div>
...
async function validateData() {
  ...
  try {
    const [webRes, mobileRes, ccRes] = await Promise.all([
      fetchWebsiteSettlement(props.year, props.webToken),
      fetchMobileSettlement(props.year, props.mobileToken),
    ...
    console.error('Settlement FS Validation Error:', error)
  } finally {
    loading.value = false
  }
}
