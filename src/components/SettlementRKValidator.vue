      ...
      <div class="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden">
        ...
        <div class="px-6 py-5 border-b border-slate-800 flex items-center justify-between bg-slate-950">
          <div>
            ...
            <p class="text-slate-500 text-sm">Bank Statement comparison by month</p>
          </div>
...
async function validateData() {
  ...
  try {
    ...
    for (let month = 1; month <= 12; month++) {
      ...
      const ccTotal = cc?.receipt_total || 0
    ...
    console.error('Bank Statement Validation Error:', error)
  } finally {
    loading.value = false
  }
}
